...
const guest_state_flag_z_field = 1;
const guest_state_io_field = 2;
const guest_state_palette_field = 3;
const guest_state_vram_field = 4;
...
fn emitPrelude(writer: *Io.Writer) Io.Writer.Error!void {
    ...
    try writer.print("%GuestState = type {{ %Registers, i1, [1024 x i8], [1024 x i8], [98304 x i8] }}\n", .{});
...
fn emitFlagZPtr(
    ...
    try writer.print(
        "  %flag_z_ptr_{x:0>8} = getelementptr inbounds %GuestState, ptr %{s}, i32 0, i32 {d}\n",
        .{ address, state_name, guest_state_flag_z_field },
    );
}
