...
fn emitPrelude(writer: *Io.Writer) Io.Writer.Error!void {
    ...
    try writer.print("%GuestState = type {{ %Registers, i1, i1, i1, i1, i32, [262144 x i8], [32768 x i8], [1024 x i8], [1024 x i8], [98304 x i8], [1024 x i8], i1, i32, i32, %BankedFiqRegisters, [131072 x i8], i32, i32, i32, i64, i1, i64 }}\n", .{});
...
fn emitInstructionBlock(writer: *Io.Writer, function: Function, node: InstructionNode) Io.Writer.Error!void {
    ...
    try emitInstructionBudgetCheck(writer, function.entry, node.address);
    try emitRetiredInstructionIncrement(writer, node.address);
...
fn emitRetiredInstructionIncrement(writer: *Io.Writer, address: u32) Io.Writer.Error!void {
    try writer.print(
        "  %retired_count_ptr_{x:0>8} = getelementptr inbounds %GuestState, ptr %state, i32 0, i32 {d}\n",
        .{ address, guest_state_retired_count_field },
    );
    try writer.print(
        "  %retired_count_{x:0>8} = load i64, ptr %retired_count_ptr_{x:0>8}, align 8\n",
        .{ address, address },
    );
    try writer.print(
        "  %retired_count_next_{x:0>8} = add i64 %retired_count_{x:0>8}, 1\n",
        .{ address, address },
    );
    try writer.print(
        "  store i64 %retired_count_next_{x:0>8}, ptr %retired_count_ptr_{x:0>8}, align 8\n",
        .{ address, address },
    );
}

fn emitInstructionBudgetCheck(
    writer: *Io.Writer,
    function_entry: armv4t_decode.CodeAddress,
    address: u32,
) Io.Writer.Error!void {
    try writer.print(
        "  %stop_flag_ptr_{x:0>8} = getelementptr inbounds %GuestState, ptr %state, i32 0, i32 {d}\n",
        .{ address, guest_state_stop_flag_field },
    );
    try writer.print("  %stop_flag_{x:0>8} = load i1, ptr %stop_flag_ptr_{x:0>8}, align 1\n", .{ address, address });
    try writer.print(
        "  br i1 %stop_flag_{x:0>8}, label %stop_return_{x:0>8}, label %budget_check_{x:0>8}\n",
        .{ address, address, address },
    );
    try writer.print("stop_return_{x:0>8}:\n", .{address});
    try writer.print("  br label %guest_return_{s}_{x:0>8}\n", .{
        instructionSetName(function_entry.isa),
        function_entry.address,
    });
    try writer.print("budget_check_{x:0>8}:\n", .{address});
    try writer.print(
        "  %instruction_budget_ptr_{x:0>8} = getelementptr inbounds %GuestState, ptr %state, i32 0, i32 {d}\n",
        .{ address, guest_state_instruction_budget_field },
    );
    try writer.print(
        "  %instruction_budget_{x:0>8} = load i64, ptr %instruction_budget_ptr_{x:0>8}, align 8\n",
        .{ address, address },
    );
    try writer.print(
        "  %instruction_budget_unlimited_{x:0>8} = icmp eq i64 %instruction_budget_{x:0>8}, -1\n",
        .{ address, address },
    );
    try writer.print(
        "  br i1 %instruction_budget_unlimited_{x:0>8}, label %budget_continue_{x:0>8}, label %budget_finite_{x:0>8}\n",
        .{ address, address, address },
    );
    try writer.print("budget_finite_{x:0>8}:\n", .{address});
    try writer.print(
        "  %instruction_budget_empty_{x:0>8} = icmp eq i64 %instruction_budget_{x:0>8}, 0\n",
        .{ address, address },
    );
    try writer.print(
        "  br i1 %instruction_budget_empty_{x:0>8}, label %budget_stop_{x:0>8}, label %budget_decrement_{x:0>8}\n",
        .{ address, address, address },
    );
    try writer.print("budget_stop_{x:0>8}:\n", .{address});
    try writer.print("  store i1 true, ptr %stop_flag_ptr_{x:0>8}, align 1\n", .{address});
    try writer.print("  br label %guest_return_{s}_{x:0>8}\n", .{
        instructionSetName(function_entry.isa),
        function_entry.address,
    });
    try writer.print("budget_decrement_{x:0>8}:\n", .{address});
    try writer.print(
        "  %instruction_budget_next_{x:0>8} = sub i64 %instruction_budget_{x:0>8}, 1\n",
        .{ address, address },
    );
    try writer.print(
        "  store i64 %instruction_budget_next_{x:0>8}, ptr %instruction_budget_ptr_{x:0>8}, align 8\n",
        .{ address, address },
    );
    try writer.print("  br label %budget_continue_{x:0>8}\n", .{address});
    try writer.print("budget_continue_{x:0>8}:\n", .{address});
}
